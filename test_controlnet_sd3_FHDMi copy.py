...
def get_parser():
    parser = argparse.ArgumentParser(description='IMAGE DEMOIREING')
    ...
    parser.add_argument('--dataset', type=str, default='FHDMi_Large') #ex: FHDMi_Base, FHDMi_Large, DIV2K_bicubic_Base, DIV2K_bicubic_Large, DIV2K_unknown_Base, DIV2K_unknown_Large
    parser.add_argument('--save_path', type=str, default='FHDMi1000LL')
    ...
    parser.add_argument('--output_folder', type=str, default='/scratch/ll5484/lillian/GM/dataset/FHDMi/test')
    parser.add_argument('--experiments', type=json.loads, default=json.dumps([
        { "tag": "exp1_z2x_condY", "source": "z", "condition": "lr", "eval_ckp": [] },
        { "tag": "exp2_z2x_condNone", "source": "z", "condition": "null", "eval_ckp": [90] },
...
class PairedImageDataset(Dataset):
    def __init__(self, root, dataset, mode):
        ...
        y = sorted(os.listdir(self.y_dir))
        z = sorted(os.listdir(self.z_dir))
        x = sorted(os.listdir(self.x_dir))

        random.seed(24)
        indices = list(range(len(y)))
        random.shuffle(indices)

        self.filenames_y = [y[i] for i in indices]
        self.filenames_z = [z[i] for i in indices]
        self.filenames_x = [x[i] for i in indices]
